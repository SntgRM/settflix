...
class BuscarPeliculasServiceTests(TestCase):
    ...
    def test_busqueda_exitosa(self, mock_get, mock_getenv):
        ...
        mock_response.json.return_value = {
            'Response': 'True',
            'Search': [
                {'imdbID': 'tt0372784', 'Title': 'Batman Begins', 'Year': '2005', 'Poster': 'https://poster.url'}
            ],
            'totalResults': '1',
        }
        ...
        self.assertEqual(data['resultados'][0]['id_pelicula'], 'tt0372784')
        self.assertEqual(data['resultados'][0]['titulo'], 'Batman Begins')

    @patch('movies.services.os.getenv', return_value='fake-key')
    @patch('movies.services.requests.get')
    def test_sin_resultados_lanza_error_controlado(self, mock_get, mock_getenv):
        mock_response = Mock()
        mock_response.status_code = 200
    ...
    @patch('movies.services.os.getenv', return_value='fake-key')
    @patch('movies.services.requests.get')
    def test_timeout_lanza_error_controlado(self, mock_get, mock_getenv):
        import requests
...
class MovieSearchViewTests(TestCase):
    ...
    def test_busqueda_exitosa_devuelve_200(self, mock_buscar):
        mock_buscar.return_value = {
            'resultados': [
                {'id_pelicula': 'tt0372784', 'titulo': 'Batman Begins', 'anio': '2005', 'poster': 'https://poster.url'}
            ],
            'total': 1,
            'pagina': 1,
        }
    ...
    @patch('movies.views.buscar_peliculas')
    def test_api_externa_falla_devuelve_502_sin_tumbar_la_app(self, mock_buscar):
        mock_buscar.side_effect = OMDbServiceError('La API externa tardó demasiado en responder.')
